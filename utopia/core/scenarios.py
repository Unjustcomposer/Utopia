...
def generate_shock_matrix(num_ticks: int, scenario_name: str, telematics_multiplier: Union[float, np.ndarray] = 1.0, seed: int = 42) -> np.ndarray:
    ...
    if scenario_name == "baseline":
        ...
        for t in range(num_ticks):
            if t < 30:
                progress = t / 30.0
                shocks[t, 2] = (1.0 + 1.5 * progress) * telematics_multiplier.mean()
            elif t < 60:
                progress = (t - 30) / 30.0
                shocks[t, 2] = (2.5 + 1.5 * progress) * telematics_multiplier.mean()
                shocks[t, 1] = -0.02
            elif t < 90:
                progress = (t - 60) / 30.0
                shocks[t, 2] = (4.0 - 2.5 * progress) * telematics_multiplier.mean()
                shocks[t, 1] = 0.04
                shocks[t, 0] = 0.001
            else:

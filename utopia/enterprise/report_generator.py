...
class ReportPDF(FPDF):
    def header(self):
        ...
        self.cell(0, 15, 'Utopia: Executive Supply Chain & Macro Report', 0, 1, 'C', fill=True)
...
def generate_pdf_report(result) -> bytes:
    ...
    # --- PAGE 6: Utopia Differentiable Moat ---
    pdf.add_page()
    pdf.set_font('Arial', 'B', 16)
    pdf.set_text_color(0, 51, 102)
    ...
    pdf.set_text_color(0, 0, 0)
    pdf.multi_cell(0, 6, 
                   "Utopia is fundamentally different from traditional ABM vendors. Because the engine is built entirely in JAX, "
    ...
    pdf.multi_cell(0, 6,
                   ...
                   "produced a ~96% tracking error. By contrast, the Utopia gradient-trained policy significantly cuts "

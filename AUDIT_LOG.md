...
| **2026-08-09** | **Spoofable Tenant Rate Limiting** | `utopia/enterprise/rate_limit.py` | `FIXED` | Extracts `tenant_id` safely from request state populated by Auth middleware. |
| **2026-08-09** | **Cosmetic SOC2 Audit Logger** | `utopia/enterprise/audit_logger.py` | `FIXED` | Implemented secure JSON logging to dedicated audit logs directory for WORM storage capability. |
